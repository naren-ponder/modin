    ...
    def apply(self, func, *args, **kwargs):
        ...
        **kwargs : dict
        ...
        if len(call_queue) > 1:
            ...
            result, length, width, ip = apply_list_of_funcs.remote(
                call_queue, oid, str(oid)
            )
        else:
            ...
            result, length, width, ip = apply_func.remote(
                oid, func, str(oid), *args, **kwargs
            )
        result = PandasOnRayDataframePartition(result, length, width, ip)
    ...
    def drain_call_queue(self):
        ...
        if len(call_queue) > 1:
            ...
            (
                ...
                self._width_cache,
                self._ip_cache,
            ) = apply_list_of_funcs.remote(call_queue, oid, oid_hash=self._identity)
        else:
            # We handle `len(call_queue) == 1` in a different way because
            # this dramatically improves performance.
            func, args, kwargs = call_queue[0]
            ...
            (
                ...
                self._width_cache,
                self._ip_cache,
            ) = apply_func.remote(oid, func, oid_hash=self._identity, *args, **kwargs)
...
def apply_func(partition, func, oid_hash=None, *args, **kwargs):  # pragma: no cover
    """
...
@ray.remote(num_returns=4)
def apply_list_of_funcs(funcs, partition, oid_hash=None):  # pragma: no cover
    """
